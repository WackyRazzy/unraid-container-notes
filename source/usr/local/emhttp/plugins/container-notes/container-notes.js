(function () {
  ...
  const CSS = `
    ...
    .cn-pencil {
      ...
      flex-shrink: 0; user-select: none; line-height: 1; transition: color 0.15s;
    }
    ...
    .cn-display {
      ...
      flex: 1; cursor: pointer; white-space: pre-wrap; word-break: break-word; line-height: 1.4;
    }
    ...
    .cn-ta {
      flex: 1; background: var(--gray-100); border: 1px solid var(--border-color);
      border-radius: 3px; color: var(--black); font-size: 11.5px; padding: 2px 6px;
      resize: none; min-height: 24px; max-height: 100px; font-family: inherit;
      line-height: 1.4; outline: none; overflow: hidden; box-sizing: border-box;
    }
    .cn-ta:focus { border-color: var(--gray-500); background: var(--gray-000); }
    .cn-save {
      font-size: 11px; padding: 2px 10px; border-radius: 3px; border: none;
      cursor: pointer; background: #4a90d9; color: #fff; flex-shrink: 0; line-height: 1.7;
    }
    .cn-save:hover { background: #3a7bc8; }
    .cn-cancel {
      font-size: 11px; padding: 2px 10px; border-radius: 3px;
      border: 1px solid var(--border-color); cursor: pointer;
      background: var(--gray-100); color: var(--gray-600); flex-shrink: 0; line-height: 1.7;
    }
  ...
  function injectAll() {
    document.querySelectorAll('tr.sortable').forEach(tr => {
      if (tr.classList.contains('cn-row') || tr.classList.contains('folder-element')) return;
      if (!isFolderHeader(tr) && !isContainerRow(tr)) return;
  ...
  function watch() {
    ...
    const obs = new MutationObserver(muts => {
      const relevant = muts.some(m =>
        [...m.addedNodes].some(n =>
          n.nodeType === 1 && !n.classList?.contains('cn-row') &&
          (n.tagName === 'TR' || n.tagName === 'TBODY' || n.querySelector?.('tr.sortable'))
        )
      );
      if (relevant) { clearTimeout(timer); timer = setTimeout(injectAll, 250); }
    });
